...
function printTicket(divName)
{
	...
	if($contCat=="import")
	{
		$strChkImprt="SELECT count(id) as rtnVal FROM sparcsn4.inv_unit
		...
		$rtnChkImprt=mysql_query($strChkImprt);
		$rowChkImprt=mysql_fetch_object($rtnChkImprt);
	...
	else if($contCat=="export")
	{
		...
		if($trStat > 0)
		{
			$strCheckPlannedQry= "SELECT count(inv_wi.gkey) as chkCount										 
									FROM sparcsn4.inv_unit inv  
									INNER JOIN sparcsn4.inv_unit_fcy_visit ON sparcsn4.inv_unit_fcy_visit.unit_gkey=inv.gkey
									INNER JOIN sparcsn4.inv_unit_yrd_visit ON inv_unit_yrd_visit.ufv_gkey=inv_unit_fcy_visit.gkey
									INNER JOIN sparcsn4.inv_wi ON inv_wi.uyv_gkey=inv_unit_yrd_visit.gkey
									WHERE inv.id ='$container_no'";
			$rtnCheckPlanned=mysql_query($strCheckPlannedQry);
			$rowCheckPlanned=mysql_fetch_object($rtnCheckPlanned);
...
<?php
	...
	else if($contCat=="export")
	{
		...
		if($trStat > 0)
		{
			...
			$rtnCheckPlanned2=mysql_query($strCheckPlannedQry2);
			$rowCheckPlanned2=mysql_fetch_object($rtnCheckPlanned2);
