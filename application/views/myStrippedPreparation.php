<?php
	...
	$dt = date('dmYgi');
	$myFile = "StrippingPreparation".$dt.".xml";
	
	if(file_exists($_SERVER['DOCUMENT_ROOT']."/myportpanel/resources/StrippingPreparation/".$myFile))
	{
			unlink($_SERVER['DOCUMENT_ROOT']."/myportpanel/resources/StrippingPreparation/".$myFile);
	}
	
	$fh= fopen($_SERVER['DOCUMENT_ROOT']."/myportpanel/resources/StrippingPreparation/".$myFile , 'a');
	...
	$query=mysql_query("SELECT DISTINCT *
					...
					g.id AS mlo,
					b.transit_state,
					(SELECT sparcsn4.srv_event.placed_by FROM sparcsn4.srv_event WHERE sparcsn4.srv_event.applied_to_gkey=a.gkey ORDER BY sparcsn4.srv_event.applied_to_gkey DESC LIMIT 1) AS placed_by,
					(SELECT ctmsmis.mis_inv_unit.arcar_id FROM ctmsmis.mis_inv_unit WHERE gkey=a.gkey) AS arcar_id,
					(SELECT sparcsn4.srv_event.placed_time FROM sparcsn4.srv_event WHERE sparcsn4.srv_event.applied_to_gkey=a.gkey ORDER BY sparcsn4.srv_event.applied_to_gkey DESC LIMIT 1) AS placed_time, 
	...
	$position = '<position loc-type="YARD" location="CGP" slot="" />'."\n";
	fwrite($fh, $position);
	$routing = ' <routing pod-1="CGP" pod-1-name="Bangladesh" destination="2591" origin="FCL">'."\n";
	fwrite($fh, $routing);
	$carrier1 = ' <carrier direction="IB" qualifier="DECLARED" mode="VESSEL" id="'.$row->arcar_id.'" />'."\n";
	fwrite($fh, $carrier1);
	$carrier2 = ' <carrier direction="IB" qualifier="ACTUAL" mode="VESSEL" id="'.$row->arcar_id.'" />'."\n";
	fwrite($fh, $carrier2);
...
if (file_exists($_SERVER['DOCUMENT_ROOT']."/myportpanel/resources/StrippingPreparation/".$myFile))
 {
  $file = $_SERVER['DOCUMENT_ROOT']."/myportpanel/resources/StrippingPreparation/".$myFile;
  $fp = fopen($file, 'rb');
