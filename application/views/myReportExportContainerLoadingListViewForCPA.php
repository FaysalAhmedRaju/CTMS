...
$sqlMloQry="SELECT DISTINCT cont_mlo as mlo
FROM ctmsmis.mis_exp_unit 
INNER JOIN ctmsmis.mis_inv_unit ON ctmsmis.mis_inv_unit.gkey=mis_exp_unit.gkey 
WHERE mis_exp_unit.preAddStat='0' AND mis_exp_unit.delete_flag='0' $cond ORDER BY cont_mlo,craine_id,ctmsmis.mis_inv_unit.id";
//echo $sqlMloQry;
...
<?php
		//ctmsmis.mis_inv_unit.freight_kind Close By Sourav Uploaded Excel/Apps Status Not Shown
...
// WHERE mis_exp_unit.preAddStat='0' and mis_exp_unit.delete_flag='0' and snx_type='0' and cont_mlo='".$rowMlo->mlo."' ".$cond." ORDER BY cont_mlo,craine_id,ctmsmis.mis_inv_unit.id");

//size and height is taken from mis_exp_unit and row where size in 20,40,45,42 is taken to match with summary - intakhab  //03-09-2018
$query=mysql_query("SELECT * FROM
(SELECT CONCAT(SUBSTRING(ctmsmis.mis_inv_unit.id,1,4),' ',SUBSTRING(ctmsmis.mis_inv_unit.id,5)) AS id,
...
INNER JOIN sparcsn4.ref_equipment ON sparcsn4.ref_equipment.gkey=sparcsn4.inv_unit_equip.eq_gkey
INNER JOIN sparcsn4.ref_equip_type ON sparcsn4.ref_equip_type.gkey=sparcsn4.ref_equipment.eqtyp_gkey
WHERE mis_exp_unit.preAddStat='0' and mis_exp_unit.delete_flag='0' and snx_type='0' and cont_mlo='".$rowMlo->mlo."' ".$cond." ORDER BY cont_mlo,craine_id,ctmsmis.mis_inv_unit.id) AS tbl WHERE size IN('20','40','45','42')");
