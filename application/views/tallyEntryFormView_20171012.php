<html>
			...
			function stateChangeValue()
				{
					...
					if (xmlhttp.readyState==4 && xmlhttp.status==200) 
					{
						...
						if(jsonData.stat[0]=="1")
						{
							...
							alert("Exchange Done.");
							//document.getElementById('btnTr').innerHTML = "Exchange Done";
							document.getElementById('btnSig').innerHTML = "Exchange Done. <a class='button' href='#popup1'  onclick='txttransfer()'><font color='white'>Upload Signature</font></a>";
						}
						else
						{
	...
	<form action="<?php echo site_url('report/updateTallyInfo');?>" method="post" onsubmit="return validateField()">
		<table width="100%" cellpadding="0">
			...
			<tr bgcolor="#2E9AFE">
				<td align="center" colspan="2"><font color="black" size ="5"><?php echo $msg;?></font></td>
			</tr>
			<?php if($assigned!=0) {?>
			<tr bgcolor="#2E9AFE">
				<td align="left" valign="middle" style="padding-top:10px;padding-left:20px;">
					<?php
							...
							$strDt = "";							
							$strDt = "select distinct wr_date,tally_sheet_number from shed_tally_info where import_rotation='$rotation' and cont_number='$cont'";
							//echo "Tst : ".$strDt;
					...
					<input class="" size="10" type="text" id="wrDate" name="wrDate" value="<?php if($numrowDt==0) echo ""; else echo $rowrcv->wr_date;?>" />
					...
					<font color="green"><b>Tally Sheet Number</b></font> : <input class="" size="20" type="text" id="tallySheetNumber" name="tallySheetNumber" value="<?php if($numrowDt==0) echo ""; else echo $rowrcv->tally_sheet_number;?>" readonly="true"/>
					<br/></h3>
				</td>
			</tr>
			<tr>
							...
							<?php
								...
								for($i=0;$i<count($rtnContainerList);$i++) { 
									...
									<tr bgcolor="#A9D0F5">
										...
										<input type="hidden" value="<?php echo $rtnContainerList[$i]['Import_Rotation_No']?>" id="rot"  name="rot" style="width:80px">
										<input type="hidden" value="<?php echo $rtnContainerList[$i]['cont_number']?>"  id="cont" name="cont" style="width:80px">
										...
										<td align="center">
											...
											<input class="chkValidation" size="5" type="text" id="rcv<?php echo $i;?>" name="rcv<?php echo $i;?>" value="<?php if($numrowrcvAll==0) echo '0.0'; else echo $rowrcv->rcv_pack;?>" onfocus="if(this.value=='0.0') this.value=''" onblur="if(this.value=='') this.value='0.0'"/>
											<script>
										...
										<td align="center">
											...
											<font size="5"><b>+</b></font><br/>
											<input class="chkValidation" size="5" type="text" id="flt<?php echo $i;?>" name="flt<?php echo $i;?>" value="<?php if($numrowfltAll==0) echo '0.0'; else echo $rowflt->flt_pack;?>" onfocus="if(this.value=='0.0') this.value=''" onblur="if(this.value=='') this.value='0.0'"/>
											<!--script>
										...
										<td align="center">
											...
											<font size="5"><b>+</b></font><br/>
											<input class="chkValidation" type="search" size="15" value="<?php echo $RcvUnit;?>" list="rcvUnit" placeholder="Pick Unit" name="rcv_unit<?php echo $i;?>">
											<datalist id="rcvUnit" >
											<?php				
												$supDtlId = $rtnContainerList[$i]['id'];
												//echo "ID : ".$supDtlId;
										...
										<?php
										if($tbl=="sup_detail")						
												$stractual_marks = "select actual_marks from shed_tally_info where igm_sup_detail_id='$supDtlId'";
												else
												$stractual_marks = "select actual_marks from shed_tally_info where igm_detail_id='$supDtlId'";
												
												$resactual_marks = mysql_query($stractual_marks);
												$rowactual_marks=mysql_fetch_object($resactual_marks);
												$actual_marks = $rowactual_marks->actual_marks;
										?>
											<textarea cols="12" rows="5" name="actualmarks<?php echo $i;?>" style="resize:none;"><?php if($rowactual_marks==0) echo ""; else echo $actual_marks; ?></textarea>
										</td>
										<!--text area "Actual Marks" ends-->
										<td>
										<?php
										if($tbl=="sup_detail")						
											$strremarks = "select remarks from shed_tally_info where igm_sup_detail_id='$supDtlId'";
										else
											$strremarks = "select remarks from shed_tally_info where igm_detail_id='$supDtlId'";
												
											$resremarks = mysql_query($strremarks);
											$rowremarks=mysql_fetch_object($resremarks);
											$remarks = $rowremarks->remarks;
										?>
			...
			<tr bgcolor="#2E9AFE">
				<td align="center">
					<table>
						<tr id="btnTr">
							<td>
								<input type="hidden" value="<?php echo count($rtnContainerList);?>"  name="tblRow" style="width:80px">
							...
							<td>
								...
								{?>
									<!--form style="display:inline" action="<?php echo site_url('ShedBillController/tallyReport/'.str_replace("/","_",$rotation).'/'.str_replace("/","_",$cont))?>" target="_blank" method="POST"-->
