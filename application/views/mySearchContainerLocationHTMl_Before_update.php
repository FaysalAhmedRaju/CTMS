				...
				<?php $cont_id=$_POST['containerLocation']; //echo $cont_id; ?>	
					<table WIDTH="95%" align="center">
						...
						<?php
							...
							$sql="SELECT ms.id, (CASE  WHEN  ms.fcy_last_pos_loctype ='YARD'   THEN      
					(CASE   WHEN     ms.category='IMPRT'      THEN  
						(case when ms.fcy_transit_state='S70_DEPARTED' then
						CONCAT('Status: ',substring(ms.fcy_transit_state,5),',Position: ',ms.fcy_position_name,',Category : ',ms.category,',Departed Time : ',ifnull(ms.fcy_time_out,''))
						else	CONCAT('Yard : ',ctmsmis.cont_yard(ms.fcy_last_pos_slot),', Block : ',ctmsmis.cont_block(ms.fcy_last_pos_slot,ctmsmis.cont_yard(ms.fcy_last_pos_slot)),
							',Position : ',CONVERT(ms.fcy_last_pos_slot USING utf8),',Category : ',ms.category,',Discharge Time : ',ifnull(ms.fcy_time_in,''))
							END )	
							ELSE    IF(ms.category='STRGE',    CONCAT('Yard:',ctmsmis.cont_yard(ms.fcy_last_pos_slot),', Block:',ctmsmis.cont_block(ms.fcy_last_pos_slot,ctmsmis.cont_yard(ms.fcy_last_pos_slot)),',Position:', 
							CONVERT(ms.fcy_last_pos_slot USING utf8),',Category:',ms.category,',Gate Out:',ifnull(ms.fcy_time_in,'')),    CONCAT('Yard:',ctmsmis.cont_yard(ms.fcy_last_pos_slot),
							', Block:',ctmsmis.cont_block(ms.fcy_last_pos_slot,ctmsmis.cont_yard(ms.fcy_last_pos_slot)),',Position:',CONVERT(ms.fcy_last_pos_slot USING utf8),',Category:',ms.category,',Gate In:',ifnull(ms.fcy_time_in,''))    
							)
										
							END
						
					) 
				ELSE (CASE   WHEN    ms.fcy_last_pos_loctype ='VESSEL'   THEN    
					(CASE   WHEN      ms.category='EXPRT'     THEN      CONCAT('Position : ',CONVERT(ms.fcy_position_name USING utf8),',Vessel Name : ',ms.vsl_name,',Category : ',ms.category,',Load Time : ',ifnull(ms.fcy_time_out,''))
							ELSE      CONCAT('Position : ',CONVERT(ms.fcy_position_name USING utf8),',Vessel Name : ',ms.vsl_name,',Category : ',ms.category)
					END    
					)  
					ELSE (CASE     WHEN      category = 'STRGE' OR category = 'IMPRT'     THEN      CONCAT('NOT IN YARD,GATE OUT : ',ifnull(ms.fcy_time_out,''),',Category : ', CONCAT (ms.category ,', ',(SELECT CONCAT (CASE  
WHEN sub_type='DE' THEN 'Dray Off' 
...
WHERE unit_gkey=ms.gkey))  )    
ELSE      (      CASE       WHEN        category = 'IMPRT'       THEN        CONCAT('GATE IN : ',ifnull(ms.fcy_time_out,''),',Category : ',ms.category)
ELSE       (       CASE        WHEN         ms.fcy_last_pos_loctype IS NULL        THEN         'NOT FOUND'        ELSE         ''       END       )      END      )    END    )
END
)
END 
) AS dsc
FROM
ctmsmis.mis_inv_unit ms  WHERE ms.id='$cont_id' and ms.fcy_last_pos_loctype in ('YARD','VESSEL') ORDER BY ms.gkey DESC LIMIT 1";
							//echo $sql;
