...
<?php
	...
	$query=mysql_query("select id,size,height,freight_kind,mlo,mlo_name,stowage_pos,pod,seal_no,mis_exp_unit.goods_and_ctr_wt_kg as goods_and_ctr_wt_kg,truck_no,craine_id from ctmsmis.mis_exp_unit inner join ctmsmis.mis_inv_unit on mis_inv_unit.gkey=mis_exp_unit.gkey where mis_exp_unit.vvd_gkey='$vvdGkey' AND mis_exp_unit.preAddStat='0' and snx_type=0 ORDER BY freight_kind ");
...
<tr align="center">
		...
		<td><?php if($row->stowage_pos) echo $row->seal_no; else echo "&nbsp;";?></td>
		
		<td><?php if($row->stowage_pos) echo $row->goods_and_ctr_wt_kg; else echo "&nbsp;";?></td>
		<td><?php if($row->stowage_pos) echo $row->pod; else echo "&nbsp;";?></td>
		<td><?php if($row->stowage_pos) echo $row->truck_no; else echo "&nbsp;";?></td>
		<td><?php if($row->stowage_pos) echo $row->craine_id; else echo "&nbsp;";?></td>
...
where  mis_exp_unit.vvd_gkey='$vvdGkey'
) as tmp");
